...
describe('HumanServiceAgentsTable', () => {
  ...
  describe('Sorting functionality', () => {
    ...
    it('sorts by custom status fields in expansive mode', () => {
      const expansiveStore = createMockStore({
        agentsColumnsFilter: {
          visibleColumns: ['in_progress', 'closeds', 'column1', 'column2'],
        },
      ...
      const column1Index = columnIndex + visibleColumns.indexOf('column1');

      const headers = expansiveWrapper.vm.formattedHeaders;
      const column1Header = headers[column1Index].content;

      expansiveWrapper.vm.sort = { header: column1Header, order: 'asc' };
      const sortedItems = expansiveWrapper.vm.sortItems([
        ...mockItemsExpansive,
      ]);

      expect(sortedItems[0].agent).toBe('Alice');
      expect(sortedItems[1].agent).toBe('Bob');
      expect(sortedItems[2].agent).toBe('Charlie');
      expect(sortedItems[3].agent).toBe('Marcus');
      ...
      expect(sortedItemsDesc[0].agent).toBe('Marcus');
      expect(sortedItemsDesc[2].agent).toBe('Bob');
      expect(sortedItemsDesc[1].agent).toBe('Charlie');
      expect(sortedItemsDesc[3].agent).toBe('Alice');
